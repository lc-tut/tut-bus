...
- ~~`timetable-sync.yml`のデフォルト切り替え~~ → Issue #213でチーム合意が得られたため実施済み（`workflow_dispatch`に`extractor`入力を追加し、手動実行時は`gemini`にも切り替えられるようにした。フォールバックが未実装スタブのため、人が手を動かせる逃げ道として残している）
- Geminiパスの削除（コードは残置。上記の手動フォールバックとして当面維持）
