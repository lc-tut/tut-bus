  ...
  return (
    ...
    >
      <TableCell
        className={cn(
          'text-sm',
          bus.segmentType === 'shuttle' && 'bg-purple-100/40 dark:bg-purple-900/40 border-l-2 border-purple-500 dark:border-purple-600'
        )}
      >
        <div className="flex items-center gap-1.5 px-4">
          <span className={bus.segmentType === 'shuttle' ? 'font-medium text-purple-700 dark:text-purple-300' : ''}>
            {bus.destination.stopName}
          </span>
        </div>
      </TableCell>

      <TableCell className={cn('font-mono', bus.segmentType === 'shuttle' && 'bg-purple-100/40 dark:bg-purple-900/40')}>
      ...
      <TableCell className={cn('text-right', bus.segmentType === 'shuttle' && 'bg-purple-100/40 dark:bg-purple-900/40')}>
        <div className="flex items-center justify-end gap-1.5">
          ...
          {bus.segmentType === 'shuttle' ? (
            <div className="flex items-center gap-1.5">
              ...
              {/* 始発バッジ */}
              {bus.isFirstBus && (
                <Badge variant="default" className="font-semibold bg-blue-400 dark:bg-blue-600 text-white">
                  始
                </Badge>
              )}
              ...
              {/* 最終便バッジ */}
              {bus.isLastBus && (
                <Badge variant="default" className="font-semibold bg-red-400 dark:bg-red-600 text-white">
                  終
                </Badge>
              )}
              <Badge
